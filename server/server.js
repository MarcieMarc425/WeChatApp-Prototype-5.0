...
io.on('connect', socket => {
    ...
    socket.on('request-pairing', (res, callback) => {
        ...
        // Player is not in queue, adds to queue
            ...
            // Current waiting queue has 2 players, pair them
                ...
                for(var i = 0; i < pairingQueue.length; i++) {
                    io.to(pairingQueue[i].id).emit('pairing-success', 'ready');
    ...
    //     if(socket.id == p1.id)
    //         movePlayerPos(p1.id, res);
    //     else
    //         movePlayerPos(p2.id, invertPlayerAction(res));
    ...
    //     if(socket.id != p1.id) 
    //         io.to(p1.id).emit('display-otherPlayerPos', p2Pos);
    //     else
    //         io.to(p2.id).emit('display-otherPlayerPos', invertPlayerPosView(p1Pos));
    ...
    socket.on('cancel-pairing', (header, res) => {
        for(var i = 0; i < pairingQueue.length; i++) {
            if(pairingQueue[i].id == socket.id)
                pairingQueue.splice(i, 1);
        }
    ...
    socket.on('disconnect', res => {
        ...
        for(var i = 0; i < connectedQueue.length; i++) {
            if(connectedQueue[i] == socket.id)
                connectedQueue.splice(i, 1);
        }
        for(var i = 0; i < pairingQueue.length; i++) {
